  ...
  <div class="page">
    <div class="select-boss">
      <div class="account-tit">
        <router-link :to="`/account/withdrawOrder/fbWithdrawOrderDetail?id=${id}`">{{$t('selectBoss.text2')}}</router-link>
        <span class="el-icon-arrow-right"></span>
        <span>{{$t('selectBoss.text3')}}</span>
      </div>
      <mob-header :to="`/account/withdrawOrder/fbWithdrawOrderDetail?id=${id}`">{{$t('selectBoss.text3')}}</mob-header>
...
export default {
  ...
  methods: {
    getData () {
      this.$fetch.post('/bossDraw/matchListByDraw', {
        orderid: this.id
      }).then(res => {
        this.load = false
        if (res.data.success) {
    ...
    submit () {
      this.loading = true
      this.$fetch.post('/bossDraw/chooseBoss', {
        id: this.id,
        ...
        if (res.data.success) {
          this.$dialog({
            ...
            okFn: () => {
              this.$router.replace(`/account/withdrawOrder/fbWithdrawOrderDetail?id=${this.id}`)
            }
          })
        } else {
          this.$dialog({
