<template>
  <div class="page">
    <div class="order">
      <div class="account-tit">{{$t('shareRechargeOrder.text1')}}</div>
      <mob-header>{{$t('shareRechargeOrder.text1')}}</mob-header>
      <div class="web-pc">
        <div class="filter">
          <div class="search">
          ...
          <div class="order-menu">
            <div class="order-menu-item" @click="filterOrder(item.value)" :class="{active: status === item.value || (item.value === 9999 && queryType === 2)}" v-for="item in $t('shareRechargeOrder.menu')" :key="item.label">{{item.label}}</div>
          </div>
        </div>
        <div class="border-bg-table">
          <div class="table click">
            <table class="share-table">
              <thead>
                <tr>
                  <td v-for="item in $t('shareRechargeOrder.table1')" :key="item">
                    {{item}}
                  </td>
                </tr>
              </thead>
              <tbody>
                <tr v-for="item in list" :key="item.id">
                  ...
                  <td>{{item.username}}</td>
                  <td>{{status2Text(item, true, true)}}</td>
                  <td>{{$changeDate(item.createAt)}}</td>
                  <td>
                    <span v-if="!item.problemId">
                      <span v-if="item.status === 0 && item.payStatus === 11">
                      ...
                      <span v-if="item.payStatus === 13">
                        <router-link class="step-btn" :to="`/account/appeal?id=${item.id}&type=1&isBoss=1`">{{$t('orderAction.text9')}}</router-link>
                        <span class="step-btn" @click="submit(item.id, 4)">{{$t('orderAction.text3')}}</span>
                      </span>
                    </span>
                    <router-link class="step-btn" :to="`sharerRechargeOrder/sharerRechargeOrderDetail?id=${item.id}`">{{$t('common.detail')}}</router-link>
                  </td>
                </tr>
              </tbody>
            </table>
        ...
        <div class="gap">
          <div class="border-bg-table">
            <div class="mock-table">
              <router-link tag="div" class="tr" :to="`sharerRechargeOrder/sharerRechargeOrderDetail?id=${item.id}`"  v-for="item in list" :key="item.id">
                <div class="row">
                  <div class="td">{{item.depositAmount | f4}}{{item.systemCurrency}}</div>
                  <div class="td right-fold">{{status2Text(item, true, true)}}</div>
                </div>
                <div class="row">
